...
void SynthesisItemWidget::paintEvent(QPaintEvent *event)
{
    ...
    painterPath.addRoundedRect(rect().marginsRemoved(QMargins(widgetMarginX, 0, widgetMarginX, 0)),
                               cornerRadius,
                               cornerRadius);
    ...
    QRectF fileIconRect = QRectF(
                rect().left() + widgetMarginX + iconPositionX,
                rect().top() + (rect().height() - iconHeight) / 2.0,
                iconWidth,
                iconHeight);
